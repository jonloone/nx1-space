...
def process_gpm_files():
    ...
    for i, nc_file in enumerate(nc_files):
        ...
        try:
            ...
            # Get data - note the order is (time, lon, lat)
            precip = dataset.variables['precipitation'][0, :, :]  # Get first time step
            ...
            for region in regions:
                ...
                if len(lat_indices) > 0 and len(lon_indices) > 0:
                    # Extract regional precipitation using numpy indexing
                    # Create meshgrid of indices
                    lon_idx_grid, lat_idx_grid = np.meshgrid(lon_indices, lat_indices)
                    regional_precip = precip[lon_idx_grid, lat_idx_grid]
                    
                    # Calculate statistics (convert from mm/hr to mm/month)
                    # Assuming average hours in a month = 730
                    hours_in_month = 730
                    mean_precip = np.nanmean(regional_precip) * hours_in_month
                    max_precip = np.nanmax(regional_precip) * hours_in_month
                    std_precip = np.nanstd(regional_precip) * hours_in_month
                    ...
                    all_data.append({
                        ...
                        'std_precipitation_mm': std_precip,
                        'percentile_95': np.nanpercentile(regional_precip, 95) * hours_in_month,
                        'percentile_5': np.nanpercentile(regional_precip, 5) * hours_in_month
                    })
...
def create_weather_patterns_from_gpm(df):
    ...
    if df.empty:
        print("\n❌ No data to create weather patterns")
        return pd.DataFrame()
    ...
    for _, row in df.iterrows():
        # Heavy precipitation events (using 95th percentile as threshold)
        if row['mean_precipitation_mm'] > row['percentile_95'] or row['anomaly_percent'] > 50:
        ...
        # Drought conditions (using 5th percentile as threshold)
        elif row['mean_precipitation_mm'] < row['percentile_5'] or row['anomaly_percent'] < -40:
            weather_patterns.append({
    ...
    if weather_patterns:
        ...
        summary = {
            ...
            "notes": [
                ...
                "Weather patterns based on statistical anomalies and percentiles",
...
def main():
    ...
    if not processed_df.empty:
        ...
        print("\n📊 Precipitation Statistics by Region:")
        print("-" * 60)
        region_stats = processed_df.groupby('region')['mean_precipitation_mm'].agg(['mean', 'std', 'min', 'max'])
        print(region_stats.round(1))
        
        # Create weather patterns
        weather_patterns = create_weather_patterns_from_gpm(processed_df)
        
        if not weather_patterns.empty:
            # Show sample results
            ...
            for _, pattern in heavy.iterrows():
                ...
                print(f"   Precipitation: {pattern['precipitation_mm']:.0f}mm")
