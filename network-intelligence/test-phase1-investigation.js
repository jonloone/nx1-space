...
const demoData = require('./lib/demo/investigation-demo-data')

test('Demo data generator exports main function', () => {
  assertExists(demoData.generateOperationNightfallData, 'generateOperationNightfallData function should exist')
})

test('Demo data generator exports heatmap function', () => {
  assertExists(demoData.generateHeatmapData, 'generateHeatmapData function should exist')
})

test('Demo data generator exports pattern analysis function', () => {
  assertExists(demoData.analyzePatterns, 'analyzePatterns function should exist')
})

const testData = demoData.generateOperationNightfallData()
...
test('Generated data has tracking points', () => {
  ...
  assert(testData.trackingPoints.length > 100, `Should have at least 100 tracking points (got ${testData.trackingPoints.length})`)
})
...
test('Heatmap data generation works', () => {
  const heatmapData = demoData.generateHeatmapData(testData.locationStops)
...
test('Pattern analysis works', () => {
  const analysis = demoData.analyzePatterns(testData.locationStops)
...
const presets = require('./lib/config/layerPresets')

test('Investigation intelligence preset exists', () => {
  assertExists(presets.LAYER_PRESETS['investigation-intelligence'], 'Investigation intelligence preset should exist')
})

const investigationPreset = presets.LAYER_PRESETS['investigation-intelligence']
...
test('Preset helper functions work', () => {
  const comingSoon = presets.getComingSoonPresets()
...
const layerCatalog = require('./lib/config/layerCatalog')

test('Investigation layers exist in catalog', () => {
  assertExists(layerCatalog.LAYER_CATALOG['intel-movement-path'], 'intel-movement-path should exist in catalog')
  assertExists(layerCatalog.LAYER_CATALOG['intel-location-markers'], 'intel-location-markers should exist in catalog')
  assertExists(layerCatalog.LAYER_CATALOG['intel-frequency-heatmap'], 'intel-frequency-heatmap should exist in catalog')
})

test('Movement path layer has correct properties', () => {
  const layer = layerCatalog.LAYER_CATALOG['intel-movement-path']
...
test('Location markers layer has correct properties', () => {
  const layer = layerCatalog.LAYER_CATALOG['intel-location-markers']
...
test('Frequency heatmap layer has correct properties', () => {
  const layer = layerCatalog.LAYER_CATALOG['intel-frequency-heatmap']
...
test('Investigation layers have legal disclaimers', () => {
  const movementPath = layerCatalog.LAYER_CATALOG['intel-movement-path']
  const locationMarkers = layerCatalog.LAYER_CATALOG['intel-location-markers']
  const heatmap = layerCatalog.LAYER_CATALOG['intel-frequency-heatmap']
...
test('Layer catalog helper functions work', () => {
  const opsLayers = layerCatalog.getLayersByCategory('operations-intelligence')
...
test('Investigation template is registered', () => {
  assertExists(templates.investigationIntelligenceTemplate, 'Investigation intelligence template should be exported')
})

const template = templates.investigationIntelligenceTemplate
...
test('Template has correct basic properties', () => {
  ...
  assertEqual(template.config.icon, '🔍', 'Template icon should match')
})

test('Template has supported entity types', () => {
  assertExists(template.config.supportedEntityTypes, 'Template should have supported entity types')
  assert(Array.isArray(template.config.supportedEntityTypes), 'Entity types should be an array')
  assert(template.config.supportedEntityTypes.includes('person-of-interest'), 'Should support person-of-interest')
  assert(template.config.supportedEntityTypes.includes('location'), 'Should support location')
})

test('Template has default entity properties', () => {
  assertExists(template.config.defaultEntityProperties, 'Template should have default entity properties')
  assertExists(template.config.defaultEntityProperties.subjectId, 'Should have subjectId property')
  assertExists(template.config.defaultEntityProperties.caseNumber, 'Should have caseNumber property')
  assertExists(template.config.defaultEntityProperties.legalAuthorization, 'Should have legalAuthorization property')
})

test('Template has data sources', () => {
  assertExists(template.config.dataSources, 'Template should have data sources')
  assert(Array.isArray(template.config.dataSources), 'Data sources should be an array')
  assert(template.config.dataSources.length >= 4, `Should have at least 4 data sources (got ${template.config.dataSources.length})`)
})

test('Template includes tracking data source', () => {
  const trackingSource = template.config.dataSources.find(s => s.id === 'tracking-data')
...
test('Template has default layers', () => {
  assertExists(template.config.defaultLayers, 'Template should have default layers')
  assert(Array.isArray(template.config.defaultLayers), 'Layers should be an array')
  assert(template.config.defaultLayers.length >= 4, `Should have at least 4 layers (got ${template.config.defaultLayers.length})`)
})

test('Template includes movement path layer', () => {
  const pathLayer = template.config.defaultLayers.find(l => l.id === 'movement-path')
...
test('Template has UI configuration', () => {
  assertExists(template.config.ui, 'Template should have UI config')
  assertExists(template.config.ui.defaultViewport, 'Should have default viewport')
  assertExists(template.config.ui.showTimeline, 'Should have timeline setting')
  assertEqual(template.config.ui.showTimeline, true, 'Timeline should be enabled')
  assertEqual(template.config.ui.showAlerts, true, 'Alerts should be enabled')
})

test('Template default viewport is NYC', () => {
  const viewport = template.config.ui.defaultViewport
...
test('Template has features enabled', () => {
  assertExists(template.config.features, 'Template should have features')
  assertEqual(template.config.features.realTimeTracking, true, 'Real-time tracking should be enabled')
  assertEqual(template.config.features.historicalPlayback, true, 'Historical playback should be enabled')
  assertEqual(template.config.features.alerts, true, 'Alerts should be enabled')
  assertEqual(template.config.features.analytics, true, 'Analytics should be enabled')
})

test('Template is in registry', () => {
  const registeredTemplate = templates.templateRegistry.get('investigation-intelligence')
