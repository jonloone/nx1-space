...
export interface H3HexagonOpportunity {
  ...
  isCoastal: boolean;
  terrainSuitability: number; // 0-100
  ...
  competitorCount25km: number;
  competitorCount100km: number;
  
  // Geographic Context
  country: string | null;
  region: 'Northern' | 'Equatorial' | 'Southern';
  populationDensityCategory: 'urban' | 'suburban' | 'rural' | 'remote';
  ...
  buildingComplexity: 'low' | 'medium' | 'high';
  regulatoryComplexity: 'low' | 'medium' | 'high';
}

export interface H3GridGenerationOptions {
  resolutions: number[]; // H3 resolutions to generate (4-7)
  bounds?: {
    minLat: number;
    maxLat: number;
    minLon: number;
  ...
  minLandCoverage: number; // Minimum land coverage % to include hex
  maxHexagons?: number; // Maximum hexagons per resolution
  includeCoastalOnly?: boolean; // Only include coastal hexagons
}
...
export class H3GridService {
   ...
   */
  generateOpportunityGrid(options: H3GridGenerationOptions): Map<number, H3HexagonOpportunity[]> {
    const results = new Map<number, H3HexagonOpportunity[]>();
   ...
   */
  private generateHexagonsForResolution(
    resolution: number,
    options: H3GridGenerationOptions
  ): H3HexagonOpportunity[] {
    const hexagons: H3HexagonOpportunity[] = [];
    ...
    for (let lat = bounds.minLat; lat <= bounds.maxLat && processedCount < maxHexagons; lat += latStep) {
      for (let lon = bounds.minLon; lon <= bounds.maxLon && processedCount < maxHexagons; lon += lonStep) {
        ...
        if (hexagons.find(h => h.h3Index === h3Index)) continue;
        ...
        if (landCoverage < options.minLandCoverage) continue;
        
        // Skip coastal-only filter if needed
        if (options.includeCoastalOnly && !isCoastalArea(centerLat, centerLon)) continue;
        
        // Calculate hex area
        const areaKm2 = cellArea(h3Index, 'km2');
        
        // Create opportunity analysis
        const opportunity = this.analyzeHexagonOpportunity(
          h3Index,
          resolution,
          centerLat,
          centerLon,
   ...
   */
  private analyzeHexagonOpportunity(
    h3Index: string,
    resolution: number,
    centerLat: number,
    centerLon: number,
    ...
    const investmentAnalysis = this.calculateInvestmentMetrics(
      areaKm2,
      terrainSuitability,
      isCoastal,
      populationCategory,
   ...
   */
  private calculateHexLandCoverage(boundary: Array<[number, number]>): number {
   ...
   */
  private analyzeCompetition(lat: number, lon: number) {
   ...
   */
  private calculateCompetitionScore(competitionAnalysis: any): number {
   ...
   */
  private calculateWeatherScore(lat: number, lon: number, region: string): number {
   ...
   */
  private calculateCoverageScore(lat: number, lon: number): number {
   ...
   */
  private calculateTerrainSuitability(lat: number, lon: number, isCoastal: boolean): number {
  ...
  private calculateInvestmentMetrics(
    ...
    if (areaKm2 > 200) baseInvestment = 15000000; // Large teleport
    else if (areaKm2 > 40) baseInvestment = 8000000; // Regional facility
    else if (areaKm2 > 5) baseInvestment = 6000000; // Metro facility
  ...
  // Helper methods
  private getLatStepForResolution(resolution: number): number {
    // Approximate degree steps for efficient sampling
    const steps = { 4: 4, 5: 2, 6: 1, 7: 0.5 };
    return steps[resolution as keyof typeof steps] || 1;
  }
  
  private getLonStepForResolution(resolution: number): number {
  ...
  private determineCountry(lat: number, lon: number): string | null {
    ...
    if (lat >= 50 && lat <= 71 && lon >= -10 && lon <= 2) return 'UK';
    ...
    if (lat >= 42 && lat <= 70 && lon >= -141 && lon <= -52) return 'Canada';
    if (lat >= 41 && lat <= 51 && lon >= -5 && lon <= 9) return 'France';
    if (lat >= 33 && lat <= 39 && lon >= 125 && lon <= 130) return 'South Korea';
    ...
    if (lat >= 4 && lat <= 14 && lon >= 3 && lon <= 15) return 'Nigeria';
    
    return null;
  }
  
  private determineRegion(lat: number): 'Northern' | 'Equatorial' | 'Southern' {
    if (lat > 23.5) return 'Northern';
    if (lat < -23.5) return 'Southern';
    return 'Equatorial';
  }
  
  private determinePopulationCategory(lat: number, lon: number, areaKm2: number): 'urban' | 'suburban' | 'rural' | 'remote' {
    ...
    if (minDistance < 25 && areaKm2 < 10) return 'urban';
    if (minDistance < 100 && areaKm2 < 50) return 'suburban';
    if (minDistance < 500) return 'rural';
    return 'remote';
  }
  
  private isMountainousRegion(lat: number, lon: number): boolean {
  ...
  private isDesertRegion(lat: number, lon: number): boolean {
  ...
  private isFloodProneRegion(lat: number, lon: number): boolean {
    ...
    return floodRegions.some(region =>
      lat >= region.minLat && lat <= region.maxLat &&
      lon >= region.minLon && lon <= region.maxLon
    );
  }
  
  private identifySpecialFactors(lat: number, lon: number, isCoastal: boolean, country: string | null): string[] {
    const factors = [];
    
    if (isCoastal) factors.push('Coastal location advantage');
    if (Math.abs(lat) > 60) factors.push('Polar satellite coverage specialist');
    if (Math.abs(lat) < 15) factors.push('Equatorial orbit access');
    if (this.isDesertRegion(lat, lon)) factors.push('Excellent weather conditions');
    if (country === 'Singapore' || country === 'UAE') factors.push('Strategic regional hub location');
  ...
  private determineBuildingComplexity(terrainSuitability: number, isCoastal: boolean): 'low' | 'medium' | 'high' {
    if (terrainSuitability > 80) return 'low';
    if (terrainSuitability > 60) return 'medium';
    return 'high';
  }
  
  private determineRegulatoryComplexity(country: string | null): 'low' | 'medium' | 'high' {
    ...
    if (lowComplexity.includes(country)) return 'low';
   ...
   */
  getTopOpportunities(
    opportunityGrid: Map<number, H3HexagonOpportunity[]>,
    count: number = 50
  ): H3HexagonOpportunity[] {
  ...
  filterOpportunities(
    ...
    criteria: {
      ...
      regions?: Array<'Northern' | 'Equatorial' | 'Southern'>;
      countries?: string[];
      minROI?: number;
    ...
    return opportunities.filter(opp => {
      if (criteria.minScore && opp.overallScore < criteria.minScore) return false;
      if (criteria.maxRiskLevel) {
        const riskLevels = ['low', 'medium', 'high', 'very_high'];
        if (riskLevels.indexOf(opp.riskLevel) > riskLevels.indexOf(criteria.maxRiskLevel)) return false;
      }
      if (criteria.regions && !criteria.regions.includes(opp.region)) return false;
      if (criteria.countries && opp.country && !criteria.countries.includes(opp.country)) return false;
      if (criteria.minROI && opp.estimatedROI < criteria.minROI) return false;
      if (criteria.maxInvestment && opp.estimatedInvestment > criteria.maxInvestment) return false;
      return true;
    });
  }
}
...
export function generateGroundStationOpportunities(options: {
  ...
  summary: {
    ...
    averageScore: number;
    opportunitiesByRegion: Record<string, number>;
  ...
  let opportunityGrid: Map<number, H3HexagonOpportunity[]>;
  ...
  if (options.globalAnalysis) {
    ...
    for (const resolution of resolutions) {
      const allHexagons: H3HexagonOpportunity[] = [];
