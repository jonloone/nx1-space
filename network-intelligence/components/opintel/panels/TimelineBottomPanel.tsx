                  ...
                  <HorizontalTimeline
                    ...
                    onEventClick={(event) => {
                      ...
                      if (selectedEventId === event.id) {
                        ...
                        // Extract location ID from event if available
                        const locationId = event.metadata?.locationId || event.id
                        selectEvent(event.id, locationId)

                        // Fly to location if coordinates available
                        if (event.location.coordinates && typeof window !== 'undefined') {
                          // Dispatch custom event for map to listen to
                          window.dispatchEvent(new CustomEvent('timeline:fly-to-location', {
                            detail: {
